...
test('returns the requested user MCP usage page for admins', async () => {
  ...
  const response = await GET(new Request('http://localhost/api/admin/users/user-1/mcp-usage'), {
...
test('rejects non-admin users without querying usage', async () => {
  parseRequestMock.mockResolvedValue({
